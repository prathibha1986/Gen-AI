...
class ChatUI {
    ...
    getPromptKeys(language, engine) {
        ...
        // No selection -> default to page object generation
        if (!isFeatureChecked && !isPageChecked && !isTestDataChecked) {
            ...
            //else promptKeys.push('PLAYWRIGHT_TYPESCRIPT_PAGE_ONLY');
            return promptKeys;
        }

           // Feature+Page+Test Data all selected
        if (isFeatureChecked && isPageChecked && isTestDataChecked) 
            {
            ...
            if (this.isJavaSelenium(lang, eng)) {
                ...
                promptKeys.push('TEST_DATA_FOR_CYPRESS_PAGE');
            } else if (this.isCSharpSelenium(lang, eng)) {
                promptKeys.push('CUCUMBER_WITH_SELENIUM_CSHARP_STEPS');
                promptKeys.push('TEST_DATA_FOR_SELENIUM_CSHARP_PAGE');
            }else {
        ...
        if (isFeatureChecked && isPageChecked) {
            if (this.isJavaSelenium(lang, eng))
                ...
                {promptKeys.push('CUCUMBER_WITH_CYPRESS_TYPESCRIPT_STEPS');}
            else { promptKeys.push('CUCUMBER_ONLY'); 
                this.addUnsupportedLanguageMessage(lang, eng); }
        ...
        // Page only
            ...
            if (this.isJavaSelenium(lang, eng)) 
                ...
                { promptKeys.push('CYPRESS_TYPESCRIPT_PAGE_ONLY');}
            else {this.addUnsupportedLanguageMessage(lang, eng);}
        }
